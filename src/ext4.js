caterwaul.module( 'ext4' ,function($) { (function( ) {var fail=function(reason) {;
return(function( ) {throw new Error(reason) } ) .call(this) } ;
return($.ext4= (function( ) {var must_be=function(x,y) {;
...
return read_inode(2) } ;
return( {real_block_size:real_block_size,inodes_per_block:inodes_per_block,block_group_size:block_group_size,block_group_count:block_group_count,block_group_offset:block_group_offset,read_block:read_block,read_block_group:read_block_group,read_inode:read_inode,root:root} ) } ) .call(this) ) ;
var sb=result;
...
return(sb.read_block(g.inode_table+ (n/sb.inodes_per_block() >>>0) ) ) .map(function(_) {return become_inode(_, (n%sb.inodes_per_block() *sb.inode_size) ,sb) } ) } ,next_free=function(bitmap) {;
return(function( ) {var byte_index= (function(xs) {var x,x0,xi,xl,xr;
for(var x=xs[0] ,xi=0,xl=xs.length,x1;
xi<xl;
 ++xi) {x=xs[xi] ;
if(x1= (x!==0xff&&xi+1) )return x1}return false} ) .call(this,bitmap) ,real_offset=byte_index-1<<3,b=bitmap[byte_index-1] ;
return( (function(xs) {var x,x0,xi,xl,xr;
for(var x=xs,xi=0,x0,xl;
x0= (b&0x80) ;
 ++xi)x= (b<<=1,x+1) ;
return x} ) .call(this,real_offset) ) } ) .call(this) } ,next_free_inode=function() {;
return(sb.read_block(g.inode_bitmap) ) .map(function(_) {return next_free(_) } ) } ,next_free_block=function() {;
return(sb.read_block(g.block_bitmap) ) .map(function(_) {return next_free(_) } ) } ;
return( {read_inode:read_inode,next_free:next_free,next_free_inode:next_free_inode,next_free_block:next_free_block} ) } ) .call(this) ) ;
var g=result;
...
d>0?i<u:i>u;
i+=s)r.push(i) ;
return r} ) ( (0) , (i.block_count) , (1) ) ) ) } ,data=function() {;
return(data_blocks() ) .map(function(_) {return(function(it) {return(it.slice(0,i.size) ) } ) .call(this, ( (function(xs) {var x,x0,xi,xl,xr;
for(var xr=new xs.constructor() ,xi=0,xl=xs.length;
xi<xl;
 ++xi)x=xs[xi] ,xr.push.apply(xr,Array.prototype.slice.call( (x) ) ) ;
return xr} ) .call(this,_) ) ) } ) } ,read_directory=function() {;
return(data() ) .map(directory_entries_in) } ,extent_root=function() {;
return become_extent_tree(i.i_block,sb) } ,data_block=function(n) {;
return extent_root() .data_block(n) } ;
return( {permission_bits:permission_bits,extents_enabled:extents_enabled,hashed_directory:hashed_directory,file_type:file_type,data_blocks:data_blocks,data:data,read_directory:read_directory,extent_root:extent_root,data_block:data_block} ) } ) .call(this) ) ;
var i=result;
return result} ,directory_entry=function(block_data,offset) {;
return(function(it) {return(it.size=function() {;
return it.record_length||8} ,it.name=it.name.substr(0,it.name_length) ) ,it} ) .call(this, ($.ext4.directory_entry.decode(block_data,offset) ) ) } ,directory_entries_in=function(block_data) {;
return(function( ) {var result= [ ] ;
return( ( ( (function(xs) {var x,x0,xi,xl,xr;
for(var x=xs,xi=0,x0,xl;
x0= (x<block_data.length-4) ;
 ++xi)x= ( (function( ) {var d=directory_entry(block_data,x) ;
return(result.push(d) ,x0=x,x+d.size() ) } ) .call(this) ) ;
return x} ) .call(this,0) ) , ( (function(xs) {var x,x0,xi,xl,xr;
for(var xr=new xs.constructor() ,xi=0,xl=xs.length,x0;
xi<xl;
 ++xi)x=xs[xi] , (x.inode) &&xr.push(x) ;
return xr} ) .call(this,result) ) ) ) } ) .call(this) } ,become_extent_tree=function(e,sb) {;
...
var e=result;
return result} ;
return( (function(it) {return($.merge($.ext4.prototype,methods() ) ) ,it} ) .call(this, ( ($.ext4.init=function(read,write) {;
return this instanceof $.ext4? (function(it) {return(it.read=read,it.write=write) ,it} ) .call(this, (this) ) :new $.ext4(read,write) } ) ) ) ) } ) .call(this) ) } ) .call(this) } ) ;
